...
`)
  ...
  if (!existsSync(wxJsonPath)) {
    die(
      `Expected WhisperX output not found at ${wxJsonPath}. Check the whisperx run above.`,
    )
  }
  ...
  if (words.length === 0)
    die(
      "WhisperX returned no word-level timestamps. Try a different model or audio file.",
    )
...
async function ttsElevenLabs(flags, stem, outDir) {
  ...
  if (!voiceId)
    die(
      "--voice <voice-id> is required for ElevenLabs. Find IDs at https://api.elevenlabs.io/v1/voices",
    )
  ...
  if (!audio_base64 || !alignment)
    die("ElevenLabs response missing audio_base64 or alignment.")
  ...
  if (words.length === 0)
    die("ElevenLabs returned no character alignment data to build word timings from.")
...
async function main() {
  ...
  if (!hasFromAudio && !hasTts) {
    console.error(
      "[caption] No mode specified. Use --from-audio or --tts. Run with --help for usage.",
    )
    process.exit(1)
  }
